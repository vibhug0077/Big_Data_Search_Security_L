"""Run the container-tested Chapter 7 and Chapter 8 simulations only."""

from __future__ import annotations

import subprocess
import sys
from pathlib import Path


ROOT = Path(__file__).resolve().parents[1]
COMPOSE = [
    "docker",
    "compose",
    "-f",
    "docker/course-dev/docker-compose.yml",
    "run",
    "--rm",
    "course-dev",
]
LABS = {
    "chapter_07_kerberos": (
        "code/kerberos_simulation.py",
        "CHAPTER 07 KERBEROS SIMULATION SMOKE TEST PASSED",
    ),
    "chapter_08_sentry_and_authorization": (
        "code/authorization_simulation.py",
        "CHAPTER 08 SENTRY AND AUTHORIZATION SIMULATION SMOKE TEST PASSED",
    ),
}


def main() -> int:
    try:
        for lab, (script, marker) in LABS.items():
            command = f"cd /workspace/labs/{lab} && python3 {script}"
            result = subprocess.run(
                COMPOSE + ["bash", "-lc", command],
                cwd=ROOT,
                check=True,
                capture_output=True,
                text=True,
            )
            if marker not in result.stdout:
                raise RuntimeError(f"{lab} did not produce its evidence marker")
            print(result.stdout.rstrip())
        print("CHAPTER 07-08 SIMULATION LABS SMOKE TEST PASSED")
        return 0
    except (OSError, subprocess.CalledProcessError, RuntimeError) as exc:
        detail = exc.stdout if isinstance(exc, subprocess.CalledProcessError) else ""
        print(f"CHAPTER 07-08 SIMULATION LABS SMOKE TEST FAILED: {exc}\n{detail}", file=sys.stderr)
        return 1


if __name__ == "__main__":
    raise SystemExit(main())
